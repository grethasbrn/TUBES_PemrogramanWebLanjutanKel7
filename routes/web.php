...
Route::prefix('dokter')->middleware(['auth', 'role:dokter'])->group(function () {
    ...
    Route::post('/api/resep/store', [ResepController::class, 'store']);
    Route::get('/api/obat', function () { return response()->json(\App\Models\Batch::where('jumlah', '>', 0)->where(fn($q) => $q->whereNull('tgl_expired')->orWhere('tgl_expired', '>', now()))->select('id', 'nama_obat', 'tipe', 'kategori', 'jumlah', 'harga', 'harga_bpjs')->orderBy('nama_obat')->get()); });
});
