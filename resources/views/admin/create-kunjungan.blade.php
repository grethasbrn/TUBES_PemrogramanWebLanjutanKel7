...
<div class="page-section active" id="sec-create-pasien">
  ...
  <div class="form-card">
    <form action="{{ route('pasien.store') }}" method="POST" id="patientForm">
      ...
      {{-- ===== TUJUAN KUNJUNGAN ===== --}}
      <div class="form-section-title">Tujuan Kunjungan <span style="font-weight:400;color:#C4B5A5">(Per Kunjungan Ini)</span></div>
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <small class="form-hint" style="font-size:11px;color:#A8998A;" id="dokter-hint">
        ...
        <div class="form-group">
          ...
          <select id="jenis_kunjungan" name="jenis_kunjungan"
            ...
            <option value="Rawat Jalan" {{ old('jenis_kunjungan', 'Rawat Jalan') == 'Rawat Jalan' ? 'selected' : '' }}>Rawat Jalan</option>
...
<script>
  ...
  poliSelect.addEventListener('change', function () {
    ...
    fetch(`/admin/kunjungan/dokter-by-poli?poli=${encodeURIComponent(poli)}`)
      ...
      .then(data => {
        if (!data.length) {
          dokterSelect.innerHTML = '<option value="">Tidak ada dokter aktif di poli ini</option>';
          dokterHint.textContent = 'Tambahkan dokter dengan spesialisasi ini terlebih dahulu.';
          dokterHint.style.color = '#e53935';
          return;
        }
        dokterSelect.innerHTML =
          '<option value="" disabled selected>— Pilih Dokter —</option>' +
          data.map(d =>
            `<option value="${d.id}">dr. ${d.nama} (${d.spesialisasi})</option>`
          ).join('');
        dokterSelect.disabled = false;
        dokterHint.textContent = `${data.length} dokter tersedia.`;
        dokterHint.style.color = '#A8998A';
      })
      .catch(() => {
        dokterSelect.innerHTML = '<option value="">Gagal memuat dokter</option>';
        dokterHint.textContent = 'Terjadi kesalahan. Coba refresh halaman.';
        dokterHint.style.color = '#e53935';
      });
  });
    ...
    fetch('/admin/pasien/cek-nik/' + nik)
      ...
      .then(data => {
        if (data.found) {
          ...
          if (data.data.jenis_kelamin) jkSelect.value = data.data.jenis_kelamin;
          ...
          // Data medis (opsional dari kunjungan sebelumnya tidak diisi otomatis)
...
<style>
  ...
  .form-hint   { font-size: .75rem; color: #A8998A; }
